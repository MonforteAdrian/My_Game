...
impl RenderChunk2d {
    ...
    pub fn get(&self, tile_pos: &TilePos) -> &Option<PackedTileData> {
        &self.tiles[super::helpers::pos_2d_to_index(tile_pos, &self.size.into())]
    }

    pub fn get_mut(&mut self, tile_pos: &TilePos) -> &mut Option<PackedTileData> {
        self.dirty_mesh = true;
        &mut self.tiles[super::helpers::pos_2d_to_index(tile_pos, &self.size.into())]
    }

    pub fn set(&mut self, tile_pos: &TilePos, tile: Option<PackedTileData>) {
        self.dirty_mesh = true;
        self.tiles[super::helpers::pos_2d_to_index(tile_pos, &self.size.into())] = tile;
    }
    ...
    pub fn prepare(&mut self, device: &RenderDevice) {
        if self.dirty_mesh {
            ...
            self.mesh.insert_attribute(
                super::render::ATTRIBUTE_POSITION,
                VertexAttributeValues::Float32x4(positions),
            );
            self.mesh.insert_attribute(
                super::render::ATTRIBUTE_TEXTURE,
                VertexAttributeValues::Float32x4(textures),
            );
            self.mesh.insert_attribute(
                super::render::ATTRIBUTE_COLOR,
